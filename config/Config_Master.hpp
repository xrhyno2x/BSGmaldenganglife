...
/* Database Related Settings */
    ...
    save_vehicle_items[] = { "pickaxe","fuelEmpty","fuelFull", "spikeStrip", "lockpick", "defuseKit","storageSmall","storageBig","redgull","coffee","waterBottle","apple","peach","tbacon","donuts","rabbit","salema","ornate","mackerel","tuna","mullet","catshark","turtle_soup","hen","rooster","sheep","goat","defibrillator","toolkit","cprKit", "goldbar", "spikeStrip" };
...
/* System Settings */
    ...
    news_broadcast_header_length = 50 //Number of characters that a header can consist of. Anything over this may clip. This depends on the font size and various other factors. Adjust with caution.
    ...
    fuelCan_refuel = 500; //Cost to refuel an empty fuel canister at the fuel station pumps. (Be wary of your buy/sell prices on fuel cans to prevent exploits...)

    /* Gang System Configurations */
    gang_price = 30000; //Gang creation price. --Remember they are persistent so keep it reasonable to avoid millions of gangs.
    gang_upgradeBase = 5000; //The base cost for purchasing additional slots in a gang
    gang_upgradeMultiplier = 2.0; //CURRENTLY NOT IN USE

    /* Housing System Configurations */
    house_limit = 5; //Maximum number of houses a player can own.
    houseGarage_buyPrice = 150000;
    houseGarage_sellPrice = 85000;
    ...
    bank_rebel = 30000; //Amount of cash in bank for new Rebel

    paycheck_cop = 3500; //Payment for cops
    paycheck_civ = 500; //Payment for civillians
    paycheck_med = 3500; //Payment for medics
    paycheck_rebel = 1500; //Payment for Rebel

    paycheck_period = 15; //Scaled in minutes
    bank_transferTax = .10; //Tax that player pays when transferring money from ATM. Tax = Amount * multiplier
    ...
    chopShop_vehicles[] = { "Car", "Air", "Armored", "Support", "ship" }; //Vehicles that can be chopped. (Can add: "Ship" and possibly more -> look at the BI wiki...)
